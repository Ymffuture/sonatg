...
// Minimal poll/quiz creation modal opened from the chat composer's attach
// menu. Self-contained — doesn't depend on the app's other modal
// primitives, just plain Tailwind consistent with the rest of the app.
...
interface PollComposerModalProps {
  chatId: string;
  onClose: () => void;
  /** Called with the new poll's id after creation, so the caller can send a "poll" message. */
  onCreated: (pollId: string) => void;
}

export function PollComposerModal({ chatId, onClose, onCreated }: PollComposerModalProps) {
      ...
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl bg-white dark:bg-[#1E1E1E] p-5 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <p className="flex items-center gap-2 text-sm font-semibold text-[#2D3436] dark:text-[#E8E8E8]">
            <HelpCircle className="h-4 w-4 text-[#E07A5F]" /> New poll
          </p>
          ...
          {options.map((opt, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 rounded-xl bg-[#F0EBE3] dark:bg-[#2A2A2A] px-3 py-2 text-sm outline-none text-[#2D3436] dark:text-[#E8E8E8]"
              />
              {options.length > 2 && (
