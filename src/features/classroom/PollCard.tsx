...
import { Card, Tag, Badge, Tooltip, Empty, Dropdown } from "antd";
import {
  CheckCircle2,
  HelpCircle,
  Users,
...
export function PollCard({ pollId, meId }: { pollId: string; meId: string }) {
  ...
  // Results stay visible to the creator (so they always have a working
  // "show results" toggle), and to everyone once the poll closes, even
  // if it was created with resultsVisible: false.
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ─── Chart + Legend ─── */}
        {resultsHiddenFromMe ? (
          <div className="mx-4 my-3 flex items-center gap-2 rounded-xl bg-black/[0.02] px-3.5 py-3 text-xs text-[#8C8C8C] dark:bg-white/[0.03]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1.5 flex items-center gap-2">
                        ...
                        <span className="shrink-0 text-[11px] tabular-nums font-semibold text-[#8C8C8C]">
                          {resultsHiddenFromMe ? <Lock className="h-3 w-3" /> : `${count} · ${pct}%`}
                        </span>
                      </div>
                    </div>
                  </div>
